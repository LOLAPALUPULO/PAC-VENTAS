...
import { apiService } from '../services/apiService'; // Keep apiService for now as it wraps Firebase auth
import { ADMIN_ROLE, SALES_ROLE, ENC_ADMIN_EMAIL, ENC_ADMIN_PASS, ENC_SALES_EMAIL, ENC_SALES_PASS, ENC_TRIGGER_ADMIN, ENC_TRIGGER_SALES, LOLA_SVG_URL } from '../constants';

interface AuthScreenProps {
  onLogin: (role: string | null) => void;
}
...
export const AuthScreen: React.FC<AuthScreenProps> = ({ onLogin }) => {
  ...
  React.useEffect(() => {
    ...
    const initAudio = () => audioService.init(); // Explicitly reference init
  ...
  const doLogin = async (email: string, password: string, expectedRole: string) => {
    ...
    setError(null);
    try {
      const result = await apiService.login(email, password); // apiService.login already uses v9
      if (result.success) {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen w-full bg-lightbg dark:bg-darkbg p-4 transition-colors duration-300 relative overflow-hidden">
      <div className="w-full max-w-md animate-pop-in flex flex-col items-center relative z-10 glass p-8 rounded-3xl shadow-2xl border border-white/20">
        ...
        {error && (
          <p className="text-accent font-bold mb-6 text-center animate-pulse bg-red-100 dark:bg-red-900/30 py-2 px-4 rounded-lg">{error}</p>
        )}

        <div className="w-full space-y-4">
          {isAdminInput && (
            <button
              onClick={() => doLogin(atob(ENC_ADMIN_EMAIL), atob(ENC_ADMIN_PASS), ADMIN_ROLE)}
              disabled={loading}
              className="btn-3d w-full py-5 rounded-2xl font-bold text-xl text-white bg-gradient-to-r from-primary to-primaryDark shadow-3d-primary active:shadow-3d-primary-active transform hover:-translate-y-1 transition-transform"
            >
          ...
          {isSalesInput && (
            <button
              onClick={() => doLogin(atob(ENC_SALES_EMAIL), atob(ENC_SALES_PASS), SALES_ROLE)}
              disabled={loading}
              className="btn-3d w-full py-5 rounded-2xl font-bold text-xl text-white bg-gradient-to-r from-secondary to-secondaryDark shadow-3d-secondary active:shadow-3d-secondary-active transform hover:-translate-y-1 transition-transform"
            >
